...
const Navbar = () => {
  ...
  return (
    <nav className="relative z-20 h-20 text-white shadow-xl bg-gradient-to-r from-gray-800 to-gray-950">
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'} fixed inset-0 bg-gray-800 bg-opacity-90 text-white p-4 z-50 transition-all duration-300 ease-in-out`}
        style={{ top: '80px' }} // Position below the navbar
      >
        <div className="flex justify-end">
          <button onClick={toggleMenu} className="text-3xl text-white">×</button> {/* Close button */}
        </div>
        ...
        <div className="flex flex-col items-center space-y-4 text-xl font-medium">
          ...
          {isAuthenticated ? (
            <>
              <Link to="/top-movies" className="block py-2 hover:text-gray-400">TopList</Link>
              <Link to="/search-movies" className="block py-2 hover:text-gray-400">Search Movies</Link>
              <Link to="/my-profile" className="block py-2 hover:text-gray-400">My Profile</Link>
              <button onClick={logout} className="block py-2 hover:text-gray-400">Logout</button>
            </>
          ) : (
            <>
              <Link to="/login" className="block py-2 hover:text-gray-400">Login</Link>
              <Link to="/signup" className="block py-2 hover:text-gray-400">Signup</Link>
            </>
          )}
        </div>
      </div>
